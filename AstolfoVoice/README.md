...
> **v0.2.0 — playable prototype, sound physics diperdalam.** Core transport +
> handshake + proximity + sound physics raytrace + playback sudah berjalan dan
> ter-build. Lihat *Status & batasan* di bawah untuk apa yang sudah solid vs yang
> masih kasar.
...
- **Diffraction** — suara membungkung di tepi obstacle. Tiang 1-block tetap kedengar
  (leak besar); dinding 2-wide mendam, bukan bisu. Gain curve halus `1/(1+extra*k)`
  (v0.2) — bukan lompatan linear abrupt.
- **Transmission** — suara tembus dinding tipis, redaman material-dependent
  (wool/leaves menyerap besar, glass sedang, stone/obsidian opaque). Leak
  `e^(-absorption*thickness)` (hukum Beer-Lambert).
- **Medium (air)** — air di jalur -> lowpass dalam + absorption high-freq
  **eksponensial per meter** (v0.2): `700 * e^(-m*0.35)` Hz. Makin jauh di air,
  makin mendam. Air juga memperpanjang jarak efektif (absorb energi).
- **Reverb** — density block solid sekitar listener + cek "atap" -> indikasi ruang
  tertutup (bergema), dipakai untuk distance bias + (Tier 2) reverb bake.
- **Air absorption distance** — high-freq hilang di udara juga (lemah) untuk jarak
  jauh > 32 block, efek halus di luar air (v0.2).
- **Tier 1** (default, murah): hasil `SoundPath` -> modulasi `distance` + whisper flag.
  Cukup untuk "tertutup / terdengar / mendam / jernih / bergema" via field distance
  client SVC.
...
- **PrivateChannel API** — player 1 & 2 bicara tak terbatas antar anggota (distance
  besar FINITE, bukan `Float.MAX_VALUE` yang bikin client NaN), sekitar tidak dengar
  (atau dengar bila `audibleNearby`). Fix v0.2.
...
# -> AstolfoVoice/build/libs/AstolfoVoice-0.2.0.jar
```
Butuh JDK 25 + Gradle 9.6.1 (wrapper disertakan). Build pertama online (tarik deps),
setelah itu `--offline`.

## Pasang
1. Drop `AstolfoVoice-0.2.0.jar` ke `plugins/`.
...
dynamic range, decode mp3/ogg/wav + streaming playback, playlist, public API +
PlaceholderAPI + PrivateChannel, build fat-jar.
